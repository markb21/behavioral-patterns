var builder = (function () {
  ...
  selection.clear = function () {
    ...
    handlers.move = function (x, y) {
        ...
        // Create newState which contains only the altered values
        // to make setState run faster.
        // If setState(state) would be used, the whole state object would be
        // traversed.
  ...
  function showControls($where) {
    ...
    // difference between attr() and .val()
      var currentState = observableElement.getState();
    $("#mx").val( currentState.data.x || 0);
    $("#my").val( currentState.data.y || 0);
    $("#mz").val( currentState.data.z || 0);
    $("#mr").val( currentState.data.rotate || 0);
    $("#ms").val( currentState.data.scale || 0);
    $("#mrx").val(currentState.data.rotateX || 0);
    $("#mry").val(currentState.data.rotateY || 0);
  ...
  function deleteContents() {
    var el = observableElement.getState().$node[0];
  ...
  function loadData() {
    ...
    //add defaults
      ...
      newState.data.x =       parseFloat(currentState.$node[0].dataset.x)       || defaults.x;
      newState.data.y =       parseFloat(currentState.$node[0].dataset.y)       || defaults.y;
      newState.data.z =       parseFloat(currentState.$node[0].dataset.z)       || defaults.z; //new
      newState.data.scale =   parseFloat(currentState.$node[0].dataset.scale)   || defaults.scale;
      newState.data.rotate =  parseFloat(currentState.$node[0].dataset.rotate)  || defaults.rotate;
      newState.data.rotateX = parseFloat(currentState.$node[0].dataset.rotateX) || defaults.rotate; //new
      newState.data.rotateY = parseFloat(currentState.$node[0].dataset.rotateY) || defaults.rotate; //new
    ...
    function Observer(){
        ...
        this.getHash = function(){
            ...
            // otherwise it can not be retrieved in a HashSet
            if(hash === ""){
                hash = CryptoJS.SHA1(JSON.stringify(object)).toString();
            }
            return hash
...
// Observables can be checked by using
    ...
    // TODO: hidden fields of objects do not get updated
            ...
            if(currentKeysNew.length > 0) {
                ...
                for(var i=0; i < currentKeysNew.length; i++){
                    ...
                    if(currentNodeNew[key] !== null && currentNodeNew[key] !== undefined && Object.keys(currentNodeNew[key]).length > 0){
                        ...
                        // Child is a leaf node, ignore if undefined
  ...
  function redraw() {
    ...
    redrawTimeout = setTimeout(function () {
      ...
      if (selection.length > 1) {
        ...
        var currentState = observableElement.getState();
        var nodes = [{}];
        var newState = { $node:nodes };

        nodes[0].dataset.scale = currentState.data.scale;
        nodes[0].dataset.rotate = currentState.data.rotate;
        nodes[0].dataset.rotateX = currentState.data.rotateX; //new
        nodes[0].dataset.rotateY = currentState.data.rotateY; //new
        nodes[0].dataset.x = currentState.data.x;
        nodes[0].dataset.y = currentState.data.y;
        nodes[0].dataset.z = currentState.data.z; //new
      ...
      config.redrawFunction(currentState.$node[0]);
      showControls(currentState.$node);
