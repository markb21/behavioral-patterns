var builder = (function () {
  ...
  'use strict';
    var observableState = newObservableState();

    var textBoxObserver = newTextBoxObserver();
    var controlsObserver = newControlsObserver();
  ...
  jQuery.fn.slidingInput = function (opts) {
      ...
      return this.each(function () {
          ...
          function mouseMove(e) {
             ...
             // var currentValue = parseInt($el.val(), 10),
              ...
              var newElValue = Math.min(options.max, Math.max(initialValue + distance, options.min));

              updateState($el, newElValue);
    ...
    $( function() {
        // Register listeners with the text boxes such that altering them also alters the state
        $("#mx, #my, #mz, #mr, #ms, #mrx, #mry"
        ).on("focusout",

        function (e) {
            var $el = $(e.target);
            console.log($el.val());
            updateState($el, $el.val());
        });
    });

  function updateState($el, newElValue) {

    var newState = {data:{}};
    if($el.attr('id') == 'mx') {

        newState.data.x = newElValue;
    }
    else if ($el.attr('id') == 'my') {

        newState.data.y = newElValue;
    }

    else if ($el.attr('id') == 'mz') {

        newState.data.z = newElValue;
    }

    else if ($el.attr('id') == 'ms') {

        newState.data.scale = newElValue;
    }

    else if ($el.attr('id') == 'mr') {

        newState.data.rotate = newElValue;
    }

    else if ($el.attr('id') == 'mrx') {

        newState.data.rotateX = newElValue;
    }

    else if ($el.attr('id') == 'mry') {

        newState.data.rotateY = newElValue;
    }
  ...
  function loadData() {
    ...
    function Observer(){
        ...
        // Observers need hash strings since they are stored in a HashSet
    ...
    function newControlsObserver(){
        ...
        function update( observable ){
            ...
            showControls(state, state.$node);
        }

        function showControls(state, $where) {
    ...
    function Observable(my){
        ...
        this.addObserver = function (observer){
            if(observer instanceof Observer){
                ...
                throw "addObserver needs an instance of Observer as input.";
            }
        }
    ...
    function newObservableState(){
        ...
        var setState = function (newState){
            ...
            // Do not notify when only the node was altered.
